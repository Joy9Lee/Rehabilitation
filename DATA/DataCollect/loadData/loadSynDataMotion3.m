...
% oriPath=[Path '新患者组'];
oriPath=[Path '新正常组'];
[fmXls,nameXls] = xlsread([Path 'profileall.xlsx']);
        ...
        for j=1:length(typeList)-2
            if strcmp(typeList(j+2).name,'肌电')==1 && EMGFLAG
                ...
                for l=1:length(fileList)-2
                    ...
                    if ~isempty(strfind(fileList(l+2).name,'motionL3'))
                        pathL{lp}=pathE{l};
                    ...
                    elseif ~isempty(strfind(fileList(l+2).name,'motionR3'))
                        pathR{rp}=pathE{l};
            ...
            if strcmp(typeList(j+2).name,'运动')==1
                ...
                for k=1:length(collTime)-2          %traverse each data as 林翰飞2\右\运动\18-2014-12-10 16-10-18
                    ...
                    pathNameR=[pathNameR fullfile(oriPath,nameList(n+2).name,sideList(i+2).name,typeList(j+2).name,collTime(k+2).name,'\')];
                    if(side=='L')
                        [index,motion] = readRes([pathNameR{k},'Result.txt']);
                ...
                if(side=='L')
                    ...
                    if min(dm)<60
                        ...
                        [DataA(n).L.flx.shdAng,DataA(n).L.flx.elbAng]= loadAng([pathNameR{y},'Result.txt'],resIndexL{y},x);
                        [DataA(n).L.flx.divS,DataA(n).L.flx.divE] = readdiv([pathNameR{y},'Result.txt'],resIndexL{y},x);
                        DataA(n).L.flx.quat= loadQuatNew(pathNameR{y},resIndexL{y},x);
...
%                        pathNameR{y}
                ...
                if(side=='R')
                    ...
                    if min(dmr)<60
                        ...
                        [DataA(n).R.flx.shdAng, DataA(n).R.flx.elbAng]= loadAng([pathNameR{y},'Result.txt'],resIndexR{y},x);
                         DataA(n).R.flx.quat = loadQuatNew(pathNameR{y},resIndexR{y},x);
                         [DataA(n).R.flx.divS, DataA(n).R.flx.divE]= readdiv([pathNameR{y},'Result.txt'],resIndexR{y},x);
%                        pathNameR{y}
                    else
                        disp('Warnning! Can not find syn data!');
                        disp(fullfile(oriPath,nameList(n+2).name,sideList(i+2).name,typeList(j+2).name));
