...
h1=plot(pm(find(cla==1),1),pm(find(cla==1),2),'ro');
...
hold on
h2=plot(pm(find(cla==2),1),pm(find(cla==2),2),'b*');
...
h3=plot(pm(find(cla==3),1),pm(find(cla==3),2),'g+');
...
for i=1:length(indi)
    ...
    h4=plot(pmU(indi(i),1),pmU(indi(i),2),'ms');
...
%plot(x0,y0,'kx')
legend([h1,h2,h3,h4,h5],'class1','class2','class3','Control','Center')
xlabel('CP1')
ylabel('CP2')
ylim([-15 20])
